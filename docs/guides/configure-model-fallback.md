# How to Configure Model Fallback in nanoinfra

Model fallback lets nanoinfra try a primary model first, then fall back to one or
...
Merge this shape into `~/.nanoinfra/config.json` and replace provider/model names
