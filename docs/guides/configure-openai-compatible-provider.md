# How to Configure an OpenAI-Compatible Provider in nanoinfra

nanoinfra can call OpenAI-compatible model providers by configuring an `apiBase`,
...
- one successful `nanoinfra agent` run
...
not already named in nanoinfra.
...
Verify the endpoint responds before debugging nanoinfra:
...
Merge this into `~/.nanoinfra/config.json`:
...
- Do not point nanoinfra at untrusted proxy endpoints for private workspaces.

## Troubleshooting

- If `curl /models` fails, fix the provider endpoint before changing nanoinfra.
- If nanoinfra says the model is unknown, check the model ID expected by the
  provider.
- If auth fails, confirm whether the provider wants Bearer auth and whether the
  key is present in the environment that starts nanoinfra.
