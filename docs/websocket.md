...
| `trustedProxyAuth.assertionHeader` | string | — | Header injected by the identity-aware proxy after successful authentication. Routing/client metadata headers (`Host`, `Forwarded`, `X-Forwarded-*`, `X-Real-IP`, `CF-Connecting-IP`) are rejected; nanoinfra trusts the remaining header's non-empty value but does not cryptographically validate it. |
...
identity-aware reverse proxy authenticates the user before connecting to nanoinfra.
...
Nanoinfra deliberately uses only `connection.remote_address` for the peer check.
It never uses `X-Forwarded-For`, `Forwarded`, `X-Real-IP`, `CF-Connecting-IP`,
or `X-Forwarded-Host` to decide whether the proxy is trusted. Nanoinfra trusts the
...
untrusted clients can connect directly to the nanoinfra listener.
...
nanoinfra over the configured loopback address and supplies a non-empty assertion.
Keep nanoinfra firewalled from untrusted clients; this configuration is not a
