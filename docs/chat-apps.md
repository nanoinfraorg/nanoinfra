...
Connect nanoinfra to Telegram, Discord, Slack, WeChat, Email, Mattermost, and
...
| Platform | Guide |
|---|---|
| Telegram | [Build a Telegram AI Agent with nanoinfra](./guides/telegram-ai-agent.md) |
| Discord | [Build a Discord AI Agent with nanoinfra](./guides/discord-ai-agent.md) |
| Slack | [Build a Slack AI Agent with nanoinfra](./guides/slack-ai-agent.md) |
| Feishu | [Build a Feishu AI Agent with nanoinfra](./guides/feishu-ai-agent.md) |
| WhatsApp | [Build a WhatsApp AI Agent with nanoinfra](./guides/whatsapp-ai-agent.md) |
| WeChat | [Build a WeChat AI Agent with nanoinfra](./guides/wechat-ai-agent.md) |
| QQ | [Build a QQ AI Agent with nanoinfra](./guides/qq-ai-agent.md) |
| Email | [Build an Email AI Agent with nanoinfra](./guides/email-ai-agent.md) |
| Mattermost | [Build a Mattermost AI Agent with nanoinfra](./guides/mattermost-ai-agent.md) |
...
If that fails, fix installation, config, provider, or model setup first with [`quick-start.md`](./quick-start.md), [`providers.md`](./providers.md), and [`troubleshooting.md`](./troubleshooting.md). Chat apps require `nanoinfra gateway` to stay running after the channel is configured.
...
Optional package installation is available to a same-machine WebUI by default. Remote browser clients cannot change the Python environment unless an administrator explicitly enables that capability. Run `nanoinfra plugins enable <channel>` locally when the guided install is unavailable.
...
> enable the channel in the same Python environment so nanoinfra installs its
...
> To turn a channel off later, run `nanoinfra plugins disable <channel>`.
> nanoinfra keeps the saved settings, but stops loading that channel after the
> next restart.

## Manual Setup Pattern

Most examples below are snippets to merge into `~/.nanoinfra/config.json`. When a snippet includes `allowFrom`, it is showing a static allowlist. For pairing-based access on supported channels, omit `allowFrom`; Slack and Mattermost also need `dm.policy` set to `"allowlist"` for DMs to issue pairing codes.
...
If `nanoinfra channels status` does not show the channel as enabled, the config snippet is in the wrong place, the channel name is misspelled, or the config file you edited is not the one nanoinfra is reading. If the channel is enabled but messages do not arrive, run `nanoinfra gateway --verbose` and compare the platform-side credentials, event permissions, and allow lists.
...
| **WhatsApp** | QR code scan (`nanoinfra channels login whatsapp`) |
| **WeChat (Weixin)** | QR code scan (`nanoinfra channels login weixin`) |
| **Feishu** | QR code scan (`nanoinfra channels login feishu`) or App ID + App Secret |
...
The configuration badge means nanoinfra found a saved token. The live connection
...
```bash
nanoinfra plugins enable telegram
```
...
```bash
nanoinfra plugins enable mochat
```

Without these dependencies, Mochat still works through HTTP polling.

**1. Ask nanoinfra to set up Mochat for you**
...
That's it — nanoinfra handles the rest!
...
If you prefer to configure manually, add the following to `~/.nanoinfra/config.json`:
...
Modern WhatsApp can deliver a sender's LID instead of their phone number. nanoinfra
...
```bash
nanoinfra plugins enable feishu
nanoinfra channels login feishu
# Use --force to create/sign in with a new bot
```

Open the printed URL or scan the QR code with Feishu/Lark on your phone. If the optional `qrcode` package is installed, nanoinfra shows a terminal QR code; otherwise it prints the login URL. nanoinfra writes `appId`, `appSecret`, `domain`, and `enabled` under `channels.feishu` in the active config file. Use `--config <path>` to update a non-default config.
...
- **Permissions**:
  ...
  - **Streaming replies** (default in nanoinfra): add **`cardkit:card:write`** (often labeled **Create and update cards** in the Feishu developer console). Required for CardKit entities and streamed assistant text. Older apps may not have it yet — open **Permission management**, enable the scope, then **publish** a new app version if the console requires it.
  - If you **cannot** add `cardkit:card:write`, set `"streaming": false` under `channels.feishu` (see below). The bot still works; replies use normal interactive cards without token-by-token streaming.
- **Events**: Add `im.message.receive_v1` (receive messages)
  - Select **Long Connection** mode (requires running nanoinfra first to establish connection)
...
> `allowFrom`: Add your open_id (find it in nanoinfra logs when you message the bot). Use `["*"]` to allow all users.
...
> - `allowFrom`: Add your openid (find it in nanoinfra logs when you message the bot). Use `["*"]` for public access.
...
```bash
nanoinfra plugins enable napcat
```
...
```bash
nanoinfra plugins enable dingtalk
```
...
> - `allowFrom`: Add the sender ID you see in nanoinfra logs for your WeChat account. Use `["*"]` to allow all users.
> - `token`: Optional. If omitted, log in interactively and nanoinfra will save the token for you.
> - `routeTag`: Optional. When your upstream Weixin deployment requires request routing, nanoinfra will send it as the `SKRouteTag` header.
> - `stateDir`: Optional. Defaults to nanoinfra's runtime directory for Weixin state.
...
```bash
nanoinfra channels login weixin --force
```
...
```bash
nanoinfra plugins enable wecom
```
