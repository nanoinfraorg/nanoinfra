...
This page collects contributor-facing notes for extending nanoinfra. User-facing setup and runtime options live in [`configuration.md`](./configuration.md).

## Adding an LLM Provider

nanoinfra uses the provider registry in `nanoinfra/providers/registry.py` as the source of truth for LLM provider metadata. Most OpenAI-compatible providers need only two changes.
...
2. Add a field to `ProvidersConfig` in `nanoinfra/config/schema.py`:
...
- `nanoinfra/audio/transcription_registry.py` owns provider names, aliases, default models, and adapter loading.
- `nanoinfra/providers/transcription.py` owns provider-specific HTTP behavior.
...
2. Add a `ProviderSpec` in `nanoinfra/providers/registry.py`.
...
3. Add an adapter class in `nanoinfra/providers/transcription.py`.
...
4. Register the adapter in `nanoinfra/audio/transcription_registry.py`.

```python
TranscriptionProviderSpec(
    name="my_stt",
    default_model="my-default-stt-model",
    adapter="nanoinfra.providers.transcription:MySTTTranscriptionProvider",
    aliases=("mystt",),
)
```
