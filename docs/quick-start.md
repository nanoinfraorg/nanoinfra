...
The recommended installer keeps nanoinfra out of the system Python environment. On a fresh local desktop, it starts the WebUI when installation finishes.
...
| `~/.nanoinfra/workspace/` | Sessions, memory, skills, automations, and generated files |
...
If the installer-started WebUI is no longer running, run `nanoinfra webui` again. Leave that terminal open; the first-run WebUI is bound to localhost, so other devices on your network cannot reach it.
...
If pip reports `externally-managed-environment`, use the recommended installer, `uv tool install nanoinfra`, `pipx install nanoinfra`, or create a virtual environment. Do not force a system-wide install.
...
| Symptom | First check |
|---|---|
| `nanoinfra: command not found` | Reuse the installer command or method-specific runner described under [Other Install Methods](#other-install-methods) |
