...
are rejected as `assertionHeader` values. Nanoinfra trusts the assertion but does
not cryptographically validate the JWT, so configure the tunnel and Access
policy carefully and do not expose the nanoinfra listener directly to untrusted
