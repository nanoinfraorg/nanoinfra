...
from nanoinfra.cli import commands as cli_commands
from nanoinfra.cli import gateway_runtime as cli_gateway_runtime
from nanoinfra.cli import provider as provider_commands
from nanoinfra.cli import terminal as cli_terminal
from nanoinfra.cli import webui as cli_webui
from nanoinfra.cli import webui_support as cli_webui_support
...
from nanoinfra.providers.factory import ProviderSnapshot, make_provider, provider_signature
from nanoinfra.providers.openai_codex_provider import _strip_model_prefix
...
from nanoinfra.session.webui_turns import WebuiTurnRoutePolicy
...
def test_onboard_fresh_install(mock_paths):
    ...
    assert "nanoinfra is ready" in result.stdout
...
def test_onboard_recommends_webui(mock_paths):
    ...
    assert result.exit_code == 0
    assert "✓ nanoinfra is ready. Run: nanoinfra webui" in result.stdout
...
def test_onboard_interactive_discard_does_not_save_or_create_workspace(mock_paths, monkeypatch):
    ...
    from nanoinfra.cli.onboard import OnboardResult

    monkeypatch.setattr(
        "nanoinfra.cli.onboard.run_onboard",
...
def test_onboard_uses_explicit_config_and_workspace_paths(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("nanoinfra.channels.registry.discover_all", lambda: {})
    ...
    assert f'nanoinfra webui -c "{resolved_config}"' in result.stdout
...
def test_provider_logout_xai_grok_removes_instance_credentials(tmp_path, monkeypatch):
    ...
    lock_path.write_text("", encoding="utf-8")
    monkeypatch.setattr(
        "nanoinfra.providers.xai_oauth.get_xai_oauth_storage_path",
        lambda: token_path,
    )
...
def test_provider_logout_paths_resolve_to_expected_files():
    ...
    from nanoinfra.providers.xai_oauth import get_xai_oauth_storage_path
...
def test_provider_login_openai_codex_passes_configured_proxy(monkeypatch):
    ...
    monkeypatch.setattr(
        "nanoinfra.config.loader.load_config",
...
def test_provider_login_xai_grok_runs_browser_flow_with_configured_proxy(monkeypatch):
    ...
    monkeypatch.setattr(
        "nanoinfra.providers.xai_oauth.get_xai_oauth_token",
    ...
    monkeypatch.setattr("nanoinfra.providers.xai_oauth.login_xai_oauth", fake_login)
...
async def test_github_copilot_provider_refreshes_client_api_key_before_chat():
    ...
    with patch("nanoinfra.providers.openai_compat_provider.AsyncOpenAI", return_value=mock_client):
...
def test_make_provider_passes_extra_headers_to_custom_provider():
    ...
    with patch("nanoinfra.providers.openai_compat_provider.AsyncOpenAI") as mock_async_openai:
        provider = make_provider(config)
...
def mock_agent_runtime(tmp_path):
    ...
    with patch("nanoinfra.config.loader.load_config", return_value=config) as mock_load_config, \
         patch("nanoinfra.config.loader.resolve_config_env_vars", side_effect=lambda c: c), \
         patch("nanoinfra.cli.agent.sync_workspace_templates") as mock_sync_templates, \
         patch("nanoinfra.providers.factory.make_provider", return_value=_fake_provider()), \
         patch("nanoinfra.cli.terminal._print_agent_response") as mock_print_response, \
         patch("nanoinfra.bus.queue.MessageBus"), \
         patch("nanoinfra.cron.service.CronService"), \
         patch("nanoinfra.cli.agent.AgentLoop.from_config") as mock_from_config:
...
def test_agent_config_sets_active_path(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("nanoinfra.cli.agent.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("nanoinfra.providers.factory.make_provider", lambda _config: _fake_provider())
    monkeypatch.setattr("nanoinfra.bus.queue.MessageBus", lambda: object())
    monkeypatch.setattr("nanoinfra.cron.service.CronService", lambda _store: object())
    ...
    monkeypatch.setattr("nanoinfra.cli.agent.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("nanoinfra.cli.terminal._print_agent_response", lambda *_args, **_kwargs: None)
...
def test_agent_uses_workspace_directory_for_cron_store(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("nanoinfra.config.loader.set_config_path", lambda _path: None)
    monkeypatch.setattr("nanoinfra.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("nanoinfra.cli.agent.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("nanoinfra.providers.factory.make_provider", lambda _config: _fake_provider())
    monkeypatch.setattr("nanoinfra.bus.queue.MessageBus", lambda: object())
    ...
    monkeypatch.setattr("nanoinfra.cron.service.CronService", _FakeCron)
    monkeypatch.setattr("nanoinfra.cli.agent.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("nanoinfra.cli.terminal._print_agent_response", lambda *_args, **_kwargs: None)
...
def test_agent_workspace_override_does_not_migrate_legacy_cron(
    ...
    monkeypatch.setattr("nanoinfra.providers.factory.make_provider", lambda _config: _fake_provider())
    monkeypatch.setattr("nanoinfra.bus.queue.MessageBus", lambda: object())
    monkeypatch.setattr("nanoinfra.config.paths.get_cron_dir", lambda: legacy_dir)
...
def test_agent_custom_config_workspace_does_not_migrate_legacy_cron(
    ...
    monkeypatch.setattr("nanoinfra.cli.agent.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr(
        "nanoinfra.cli.terminal._print_agent_response", lambda *_args, **_kwargs: None
    )
...
def test_heartbeat_has_active_tasks(content, expected):
    from nanoinfra.cli.gateway_runtime import _heartbeat_has_active_tasks
...
def test_heartbeat_skips_bundled_template():
    from nanoinfra.cli.gateway_runtime import _heartbeat_has_active_tasks
    from nanoinfra.utils.helpers import load_bundled_template
...
def test_heartbeat_target_skips_archived_webui_sessions():
    from nanoinfra.cli.gateway_runtime import _pick_heartbeat_target_from_sessions
...
def test_heartbeat_target_uses_last_channel_for_unified_session():
    from nanoinfra.cli.gateway_runtime import _pick_heartbeat_target_from_sessions
    from nanoinfra.session.keys import LAST_CHANNEL_METADATA_KEY, UNIFIED_SESSION_KEY
...
def _patch_webui_provider_ready(monkeypatch) -> None:
    monkeypatch.setattr(
        "nanoinfra.providers.factory.validate_provider_setup",
        lambda _config: None,
    )


def _patch_gateway_ports_free(monkeypatch) -> None:
    monkeypatch.setattr("nanoinfra.cli.webui._gateway_health_ready", lambda *_a, **_kw: False)
    monkeypatch.setattr("nanoinfra.cli.webui._tcp_endpoint_reachable", lambda *_a, **_kw: False)
    monkeypatch.setattr("nanoinfra.cli.webui._webui_endpoint_reachable", lambda *_a, **_kw: False)
    monkeypatch.setattr(
        "nanoinfra.cli.gateway_runtime._tcp_endpoint_reachable",
        lambda *_a, **_kw: False,
    )
    monkeypatch.setattr(
        "nanoinfra.cli.gateway_runtime._webui_endpoint_reachable",
        lambda *_a, **_kw: False,
    )
...
def _patch_cli_command_runtime(
    ...
    *,
    ...
    monkeypatch.setattr(
        "nanoinfra.cli.webui.sync_workspace_templates",
        sync_templates or (lambda _path: None),
    )
    monkeypatch.setattr(
        "nanoinfra.cli.gateway_runtime.sync_workspace_templates",
        sync_templates or (lambda _path: None),
    )
    monkeypatch.setattr(
        "nanoinfra.providers.factory.make_provider",
        provider_factory,
    )
    monkeypatch.setattr(
        "nanoinfra.providers.factory.build_provider_snapshot",
        lambda _config: _test_provider_snapshot(provider_factory(_config), _config),
    )
    monkeypatch.setattr(
        "nanoinfra.providers.factory.load_provider_snapshot",
        lambda _config_path=None: _test_provider_snapshot(provider_factory(config), config),
    )
    monkeypatch.setattr(
        "nanoinfra.cli.webui_support._provider_setup_error",
        lambda _config: None,
    )
    _patch_gateway_ports_free(monkeypatch)
    _patch_gateway_helper_processes(monkeypatch)

    if message_bus is not None:
        monkeypatch.setattr("nanoinfra.bus.queue.MessageBus", message_bus)
    if session_manager is not None:
        monkeypatch.setattr("nanoinfra.session.manager.SessionManager", session_manager)
    if cron_service is not None:
        monkeypatch.setattr("nanoinfra.cron.service.CronService", cron_service)
    if get_cron_dir is not None:
        monkeypatch.setattr("nanoinfra.config.paths.get_cron_dir", get_cron_dir)
...
def test_heartbeat_empty_response_still_retains_recent_messages(
    ...
    monkeypatch.setattr("nanoinfra.cli.gateway_runtime.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("nanoinfra.channels.manager.ChannelManager", _FakeChannelManager)
    monkeypatch.setattr("nanoinfra.cli.gateway_runtime.read_webui_sidebar_state", lambda: {})
    monkeypatch.setattr("nanoinfra.cli.gateway_runtime.evaluate_response", _unexpected_evaluator)
...
def test_webui_yes_creates_config_and_enables_local_websocket(
    ...
    monkeypatch.setattr("nanoinfra.cli.webui._run_gateway", _fake_run_gateway)
    ...
    assert "nanoinfra is running in this terminal" in compact_output
    assert "Press Ctrl+C here to stop nanoinfra" in compact_output
...
def test_webui_yes_starts_first_run_without_provider_setup(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "nanoinfra.cli.webui._provider_setup_error",
        lambda _config: "No API key configured for provider 'custom'.",
    )
    _patch_gateway_ports_free(monkeypatch)
...
def test_webui_missing_runtime_env_fails_before_starting_gateway(
    ...
    config_file = tmp_path / "config.json"
    missing_env = "NANOINFRA_TEST_MISSING_WEBUI_SECRET"
...
def test_webui_background_starts_runtime_and_opens_browser(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "nanoinfra.cli.webui._open_webui_browser",
    ...
    assert "nanoinfra gateway status --config" in compact_output
    ...
    assert "nanoinfra gateway stop --config" in compact_output
...
def test_webui_foreground_attaches_to_existing_managed_gateway(monkeypatch, tmp_path: Path) -> None:
    ...
    _patch_webui_provider_ready(monkeypatch)
    monkeypatch.setattr("nanoinfra.cli.webui.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("nanoinfra.cli.webui._gateway_health_ready", lambda *_args, **_kwargs: True)
    monkeypatch.setattr("nanoinfra.cli.webui._webui_endpoint_reachable", lambda *_args, **_kwargs: True)
    monkeypatch.setattr(
        "nanoinfra.cli.webui._open_webui_browser",
    ...
    monkeypatch.setattr("nanoinfra.gateway.GatewayRuntime", _FakeRuntime)
    monkeypatch.setattr(
        "nanoinfra.cli.webui._attach_to_background_gateway",
...
def test_attach_to_background_gateway_stops_on_ctrl_c(monkeypatch, capsys) -> None:
    ...
    monkeypatch.setattr("nanoinfra.cli.webui_support.time.sleep", _interrupt)
    ...
    assert "Press Ctrl+C here to stop nanoinfra" in output
...
def test_webui_foreground_does_not_claim_unmanaged_gateway(monkeypatch, tmp_path: Path) -> None:
    ...
    _patch_webui_provider_ready(monkeypatch)
    monkeypatch.setattr("nanoinfra.cli.webui.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("nanoinfra.cli.webui._gateway_health_ready", lambda *_args: True)
    monkeypatch.setattr("nanoinfra.cli.webui._webui_endpoint_reachable", lambda *_args: True)
    monkeypatch.setattr("nanoinfra.cli.webui._open_webui_browser", lambda *_args, **_kwargs: None)
    monkeypatch.setattr(
        "nanoinfra.cli.webui._attach_to_background_gateway",
...
def test_webui_foreground_refuses_occupied_webui_port(monkeypatch, tmp_path: Path) -> None:
    ...
    _patch_webui_provider_ready(monkeypatch)
    monkeypatch.setattr("nanoinfra.cli.webui.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("nanoinfra.cli.webui._gateway_health_ready", lambda *_args, **_kwargs: False)
    monkeypatch.setattr("nanoinfra.cli.webui._webui_endpoint_reachable", lambda *_args, **_kwargs: True)
    monkeypatch.setattr("nanoinfra.cli.webui._tcp_endpoint_reachable", lambda *_args, **_kwargs: False)
    monkeypatch.setattr(
        "nanoinfra.cli.webui._run_gateway",
    ...
    assert result.exit_code == 1
    assert "nanoinfra cannot start because one of its local ports is already in use" in result.stdout
...
def _patch_serve_runtime(monkeypatch, config: Config, seen: dict[str, object]) -> None:
    ...
    monkeypatch.setattr("nanoinfra.cli.commands.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("nanoinfra.api.server.create_app", _fake_create_app)
...
def test_gateway_unbound_agent_cron_is_skipped(
    ...
    monkeypatch.setattr("nanoinfra.config.loader.set_config_path", lambda _path: None)
    monkeypatch.setattr("nanoinfra.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("nanoinfra.cli.gateway_runtime.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("nanoinfra.providers.factory.make_provider", lambda _config: provider)
    monkeypatch.setattr("nanoinfra.cli.webui_support._provider_setup_error", lambda _config: None)
    _patch_gateway_ports_free(monkeypatch)
    monkeypatch.setattr(
        "nanoinfra.providers.factory.build_provider_snapshot",
    ...
    monkeypatch.setattr("nanoinfra.bus.queue.MessageBus", lambda: bus)
    ...
    monkeypatch.setattr("nanoinfra.session.manager.SessionManager", _FakeSessionManager)
    ...
    monkeypatch.setattr("nanoinfra.cron.service.CronService", _FakeCron)
    monkeypatch.setattr("nanoinfra.cli.gateway_runtime.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("nanoinfra.channels.manager.ChannelManager", _StopAfterCronSetup)
    monkeypatch.setattr(
        "nanoinfra.cli.gateway_runtime.evaluate_response",
...
def test_gateway_local_trigger_queue_submits_agent_turns(
    ...
    monkeypatch.setattr("nanoinfra.channels.manager.ChannelManager", _FakeChannelManager)
    monkeypatch.setattr(
        "nanoinfra.triggers.local_runner.run_local_trigger_queue",
...
def test_migrate_cron_store_moves_legacy_file(tmp_path: Path) -> None:
    ...
    with patch("nanoinfra.config.paths.get_cron_dir", return_value=legacy_dir):
...
def test_gateway_health_endpoint_binds_and_serves_expected_responses(
    ...
    monkeypatch.setattr("nanoinfra.cli.gateway_runtime.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("nanoinfra.channels.manager.ChannelManager", _FakeChannelManager)
    monkeypatch.setattr("nanoinfra.cron.service.CronService", _FakeCronService)
...
def test_gateway_shutdown_event_exits_forever_runtime_tasks(
    ...
    monkeypatch.setattr("asyncio.start_server", _fake_start_server)
    monkeypatch.setattr(
        "nanoinfra.cli.gateway_runtime._install_gateway_shutdown_handlers",
