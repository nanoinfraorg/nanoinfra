...
from nanoinfra.providers.openai_responses import build_responses_state
...
def _mock_codex_token(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "nanoinfra.providers.openai_codex_provider.get_codex_token",
        fake_token,
    )
...
def _capture_codex_warnings(monkeypatch: pytest.MonkeyPatch) -> _WarningCaptureLogger:
    ...
    monkeypatch.setattr("nanoinfra.providers.openai_codex_provider.logger", capture)
    return capture
...
async def test_codex_request_honors_stream_idle_timeout_env(monkeypatch) -> None:
    """NANOINFRA_STREAM_IDLE_TIMEOUT_S overrides the default Codex stream timeout."""
    monkeypatch.setenv("NANOINFRA_STREAM_IDLE_TIMEOUT_S", "5")
    original_client = httpx.AsyncClient
    seen: dict[str, int] = {}
...
async def test_codex_provider_passes_proxy_to_oauth_and_response_request(monkeypatch) -> None:
    ...
    monkeypatch.setattr("nanoinfra.providers.openai_codex_provider.get_codex_token", fake_token)
    monkeypatch.setattr("nanoinfra.providers.openai_codex_provider._request_codex", fake_request)
...
async def test_codex_replayed_tool_turn_omits_server_item_ids(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "nanoinfra.providers.openai_codex_provider._request_codex",
        fake_request,
    )
