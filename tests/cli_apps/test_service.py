...
from nanoinfra.apps.cli.service import CliAppError, CliAppManager, CliAppsRuntimeConfig
...
def test_payload_includes_nanoinfra_extension_registry(tmp_path: Path) -> None:
    manager = _manager(tmp_path)
    _write_cache(manager._cache_path("harness"), {"meta": {"updated": "2026-04-16"}, "clis": []})
...
def test_optional_extension_registry_failure_does_not_break_payload(
    ...
    monkeypatch.setattr("nanoinfra.apps.cli.service.httpx.get", fail_get)
...
def test_run_argv_logs_command_exit_and_output(
    ...
    from nanoinfra.apps.cli import service as cli_service
...
def test_install_records_entry_point_path_and_pip_distribution(
    ...
    monkeypatch.setattr(
        "nanoinfra.apps.cli.service.importlib_metadata.distributions",
        lambda: [
            SimpleNamespace(
                entry_points=[
...
def test_fetch_skill_content_allows_cli_anything_raw_skill_url(
    ...
    monkeypatch.setattr("nanoinfra.apps.cli.service.httpx.get", fake_get)
