...
from nanoinfra.config.schema import ChannelsConfig, Config
from nanoinfra.providers.transcription import GroqTranscriptionProvider as _GroqProvider
from nanoinfra.providers.transcription import OpenAITranscriptionProvider as _OpenAIProvider
from nanoinfra.utils.restart import RestartNotice
...
def _stub_channel_registry(
    ...
    *plugins: ChannelPlugin,
    ...
    monkeypatch.setattr("nanoinfra.channels.registry.discover_plugins", discover)
...
def _stub_optional_feature_cli(
    ...
    *,
    ...
    monkeypatch.setattr("nanoinfra.optional_features.optional_dependency_groups", lambda: extras)
    monkeypatch.setattr("nanoinfra.optional_features.extra_installed", lambda _name, _deps: installed)
    if commands is not None:
        monkeypatch.setattr(
            "nanoinfra.optional_features.run_install_command",
...
def test_channel_manager_loads_descriptor_but_not_disabled_runtime(monkeypatch):
    ...
    monkeypatch.setattr(
        "nanoinfra.channels.registry._channel_package_names",
        lambda: ["fakeplugin"],
    )
    monkeypatch.setattr(
        "nanoinfra.channels.registry.load_channel_package",
...
def test_multi_plugin_action_defaults_to_default_instance(
    ...
    disabled = nanoinfra_features_action("disable", {"name": ["managedmulti"]})
...
async def test_single_channel_enable_applies_defaults_before_hot_reload(
    ...
    payload = nanoinfra_features_action("enable", {"name": ["singleplugin"]})
...
def test_discover_plugins_loads_package_descriptors():
    ...
    with (
        patch("nanoinfra.channels.registry._channel_package_names", return_value=["line"]),
        patch("nanoinfra.channels.registry.load_channel_package", return_value=plugin),
    ):
...
def test_discover_plugins_skips_names_outside_enabled_set():
    ...
    with (
        patch("nanoinfra.channels.registry._channel_package_names", return_value=["disabled"]),
        patch("nanoinfra.channels.registry.load_channel_package", side_effect=_load_disabled),
    ):
...
def test_discover_plugins_handles_load_error():
    ...
    with (
        patch("nanoinfra.channels.registry._channel_package_names", return_value=["broken"]),
        patch("nanoinfra.channels.registry.load_channel_package", side_effect=_boom),
    ):
        result = discover_plugins()
...
def test_discover_all_includes_available_channel_packages():
    from nanoinfra.channels.registry import discover_all, discover_plugins
...
def test_discover_enabled_imports_only_enabled_packages():
    from nanoinfra.channels.registry import discover_enabled
...
def test_discover_enabled_warns_for_enabled_package_import_errors():
    ...
    with patch("nanoinfra.channels.registry.logger.warning") as warning:
...
def test_manager_installs_manifest_dependencies_before_loading_enabled_channel(monkeypatch):
    ...
    monkeypatch.setattr("nanoinfra.optional_features.extra_installed", extra_installed)
    monkeypatch.setattr("nanoinfra.optional_features.install_extra", install_extra)
    config = Config.model_validate({
        "channels": {
...
def test_manager_reports_dependency_install_failure_as_runtime_failure(monkeypatch):
    ...
    _stub_channel_registry(monkeypatch, plugin)
    monkeypatch.setattr(
        "nanoinfra.optional_features.extra_installed",
...
def test_manager_loads_websocket_from_default_config():
    ...
    with patch("nanoinfra.channels.registry.discover_plugins", return_value={"websocket": plugin}):
...
async def test_base_channel_reads_current_transcription_config_each_call(
    ...
    from nanoinfra.providers import transcription as transcription_mod
...
async def test_base_channel_respects_disabled_transcription_config(
    ...
    with patch("nanoinfra.providers.transcription.GroqTranscriptionProvider") as provider:
...
def test_openai_transcription_provider_honors_api_base_argument():
    from nanoinfra.providers.transcription import OpenAITranscriptionProvider
...
async def test_transcription_provider_includes_language(tmp_path, provider_cls, language):
    ...
    with patch("nanoinfra.providers.transcription.httpx.AsyncClient", return_value=_stub_async_client(captured)):
...
def test_plugins_enable_extra_without_channel_only_installs(monkeypatch, tmp_path):
    ...
    log_flags: list[bool] = []
    config_path = tmp_path / "config.json"
    original_set_logs = cli_commands._set_nanoinfra_logs
    ...
    monkeypatch.setattr("nanoinfra.cli.commands._set_nanoinfra_logs", _set_logs)
...
def test_plugins_enable_logs_option_enables_nanoinfra_logs(monkeypatch, tmp_path):
    from typer.testing import CliRunner
...
def test_enable_optional_feature_blocks_install_when_disallowed(monkeypatch, tmp_path):
    from nanoinfra.optional_features import OptionalFeatureError, enable_optional_feature
    ...
    monkeypatch.setattr("nanoinfra.optional_features.extra_installed", lambda _name, _deps: False)
...
def test_enable_optional_feature_skips_install_when_dependency_present(
    ...
    from nanoinfra.optional_features import InstallResult, enable_optional_feature
    ...
    monkeypatch.setattr("nanoinfra.optional_features.extra_installed", lambda _name, _deps: True)
    ...
    monkeypatch.setattr("nanoinfra.optional_features.install_extra", _install_extra)
...
def test_enable_optional_feature_lazy_reader_does_not_require_restart(monkeypatch, tmp_path):
    ...
    assert payload["last_action"]["message"] == "Feature 'documents' is included with nanoinfra"
...
def test_disable_optional_feature_rejects_unknown_features_and_non_channels(
    ...
    from nanoinfra.optional_features import OptionalFeatureError, disable_optional_feature
...
def test_disable_optional_feature_writes_channel_disabled(monkeypatch, tmp_path):
    from nanoinfra.optional_features import disable_optional_feature
...
def test_live_runtime_status_overrides_enabled_configuration_for_webui():
    from nanoinfra.optional_features import with_channel_runtime_status
...
def test_package_manifest_metadata_drives_optional_feature_payload(monkeypatch):
    ...
    monkeypatch.setattr("nanoinfra.optional_features.extra_installed", record_extra)
...
def test_enable_uses_uv_when_tool_environment_has_no_pip(
    ...
    from nanoinfra import optional_features
...
def test_enable_bootstraps_pip_with_ensurepip(monkeypatch):
    ...
    assert calls == [
        [sys.executable, "-m", "pip", "install", "nanoinfra[bedrock]"],
...
async def test_send_with_retry_retries_on_failure():
    ...
    with patch("nanoinfra.channels.manager.asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
        await mgr._send_with_retry(mgr.channels["failing"], msg)
...
async def test_send_with_retry_no_retry_when_max_is_zero():
    ...
    with patch("nanoinfra.channels.manager.asyncio.sleep", new_callable=AsyncMock):
        await mgr._send_with_retry(mgr.channels["failing"], msg)
...
async def test_send_with_retry_propagates_cancelled_error_during_sleep():
    ...
    with patch("nanoinfra.channels.manager.asyncio.sleep", side_effect=cancel_during_sleep):
        with pytest.raises(asyncio.CancelledError):
            await mgr._send_with_retry(mgr.channels["failing"], msg)
...
async def test_restart_notice_retries_until_running_channel_accepts_delivery():
    ...
    with patch("nanoinfra.channels.manager._SEND_RETRY_DELAYS", (0,)):
