...
async def test_exec_blocks_wget_localhost():
    tool = ExecTool()
    with patch("nanoinfra.security.network.socket.getaddrinfo", _fake_resolve_localhost):
...
# --- #2989: block writes to nanoinfra internal state files -----------------
...
def test_exec_allows_absolute_path_inside_bwrap_ro_bind(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("nanoinfra.agent.tools.shell._IS_WINDOWS", False)
    tool = ExecTool(
        working_dir=str(workspace),
...
def test_exec_allows_workspace_paths_from_subdirectory(tmp_path):
    ...
    commands in ``~/.nanoinfra/workspace/obsidian_notes`` reference paths
