...
from nanoinfra.runtime_context import RuntimeContextBlock
...
def test_bootstrap_files_are_backed_by_templates() -> None:
    template_dir = pkg_files("nanoinfra") / "templates"
...
def test_execution_rules_reach_existing_workspace_soul(tmp_path) -> None:
    ...
    workspace = _make_workspace(tmp_path)
    legacy_soul = (
        pkg_files("nanoinfra") / "templates" / "legacy" / "SOUL.md"
...
def test_identity_has_no_behavioral_instructions(tmp_path) -> None:
    ...
    assert "You are nanoinfra" not in identity
...
def test_default_soul_template_keeps_execution_policy_in_tool_contract() -> None:
    ...
    soul = (pkg_files("nanoinfra") / "templates" / "SOUL.md").read_text(encoding="utf-8")
    contract = (
        pkg_files("nanoinfra") / "templates" / "agent" / "tool_contract.md"
    ).read_text(encoding="utf-8")
...
def test_template_memory_md_is_skipped(tmp_path) -> None:
    ...
    assert "This file is automatically updated by nanoinfra" not in prompt
