...
from nanoinfra.agent.tools.sessions import ReadSessionTool, SearchSessionsTool
from nanoinfra.bus.events import INBOUND_META_SESSION_READ_SCOPE
from nanoinfra.runtime_context import RuntimeContextBlock, append_runtime_context
...
async def test_search_sessions_reads_the_full_webui_transcript_after_compaction(
    ...
    monkeypatch.setattr("nanoinfra.webui.transcript.get_webui_dir", lambda: webui_dir)
    monkeypatch.setattr("nanoinfra.webui.session_list_index.get_webui_dir", lambda: webui_dir)
    manager = SessionManager(tmp_path)
    _save_session(
        manager,
        "websocket:history",
