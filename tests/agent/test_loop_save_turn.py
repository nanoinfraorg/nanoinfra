...
from nanoinfra.cron.session_turns import CRON_HISTORY_META, CRON_TRIGGER_META
from nanoinfra.providers.base import LLMProvider, LLMResponse, ProviderConversationState
...
from nanoinfra.session.keys import (
    LAST_CHANNEL_METADATA_KEY,
    UNIFIED_SESSION_KEY,
)
from nanoinfra.session.manager import Session
from nanoinfra.session.turn_continuation import (
...
def test_save_turn_keeps_image_placeholder_and_runtime_context() -> None:
    ...
    assert session.messages[0]["content"] == [
        {"type": "text", "text": "[image: /media/telegram/photo.jpg]"},
        {"type": "text", "text": "provider context"},
    ]
    assert public_history_message(session.messages[0])["content"] == [
        {"type": "text", "text": "[image: /media/telegram/photo.jpg]"}
...
async def test_process_message_persists_unified_session_delivery_route(tmp_path: Path) -> None:
    ...
    assert persisted.metadata[LAST_CHANNEL_METADATA_KEY] == "telegram:oc_123"
...
async def test_internal_continuation_queues_turn_without_fake_user_history(
    ...
    loop = _make_full_loop(tmp_path)
    loop.consolidator.maybe_consolidate_by_tokens = AsyncMock(return_value=False)  # type: ignore[method-assign]
    session = loop.sessions.get_or_create("telegram:c-auto")
...
async def test_internal_continuation_preserves_streaming_route_metadata(
    ...
    loop = _make_full_loop(tmp_path)
    loop.consolidator.maybe_consolidate_by_tokens = AsyncMock(return_value=False)  # type: ignore[method-assign]
    session = loop.sessions.get_or_create("telegram:c-stream")
    session.metadata[GOAL_STATE_KEY] = {
        "status": "active",
...
async def test_stop_preserves_runtime_checkpoint_for_next_turn(tmp_path: Path) -> None:
    from nanoinfra.command.builtin import cmd_stop
...
async def test_turn_after_unanswered_user_keeps_tool_call_pairing(tmp_path: Path) -> None:
    ...
    session = loop.sessions.get_or_create("telegram:c-merge")
    ...
    result = await loop._process_message(
        InboundMessage(
            channel="telegram", sender_id="u1", chat_id="c-merge", content="and another thing"
        )
    )

    assert result is not None
    loop.sessions.invalidate("telegram:c-merge")
    persisted = loop.sessions.get_or_create("telegram:c-merge")
