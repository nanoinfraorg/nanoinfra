...
def append_transcript_object(
    session_key: str, obj: dict[str, Any], *, workspace: Path | str | None = None
) -> None:
    """Append one event to the durable reasoning-pane transcript.
...
def _redacted_transcript_record(
    record: dict[str, Any], workspace: Path | str | None
) -> dict[str, Any]:
...
class WebUITranscriptRecorder:
    ...
    def __init__(self, log: Any = logger, workspace: Path | str | None = None) -> None:
        # ``workspace`` enables transcript redaction (#17). Without it this recorder writes
        # the event unredacted, so the gateway passes its workspace path.
        self._log = log
    ...
    def append(self, chat_id: str, event: dict[str, Any]) -> bool:
        try:
            ...
            # Redact here rather than inside append_transcript_object (#17). That function
            # keeps its two-argument call, because tests replace it with a double, and a new
            # keyword at the call site would break every one of those doubles.
            dup = _redacted_transcript_record(dup, self._workspace)
