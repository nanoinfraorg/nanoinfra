"""Nanoinfra optional feature helpers for WebUI Settings."""
...
from nanoinfra.webui.http_utils import query_first

QueryParams = dict[str, list[str]]


def nanoinfra_features_payload() -> dict[str, Any]:
    return optional_features_payload()


def nanoinfra_feature_instance_target(query: QueryParams) -> str | None:
    ...
    if action == "disable":
        ...
        if plugin is not None and "always_enabled" in plugin.capabilities:
            raise OptionalFeatureError(
                ...
                f"Use `nanoinfra plugins disable {name}` from a terminal if you need to disable it.",
                status=400,
            )
