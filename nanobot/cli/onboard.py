"""Interactive onboarding questionnaire for nanoinfra."""
...
from nanoinfra.cli.models import (
    format_token_count,
    get_model_context_limit,
    get_model_suggestions,
)
from nanoinfra.config.loader import get_config_path, load_config, resolve_config_env_vars
...
def _show_main_menu_header() -> None:
    ...
    body.add_row(f"{__logo__} [bold {_UI_TEXT}]nanoinfra[/] [{_UI_MUTED}]v{__version__}[/]")
...
def _handle_fallback_models_field(
    ...
    from nanoinfra.config.schema import InlineFallbackConfig
...
def _try_auto_fill_context_window(model: BaseModel, new_model_name: str) -> None:
    ...
    Note:
        This function imports AgentDefaults from nanoinfra.config.schema to get
...
def _enable_quick_start_websocket_defaults(config: Config) -> bool:
    ...
    console.print(
        f"[{_UI_MUTED}]This lets the browser UI at http://127.0.0.1:8765 connect to nanoinfra.[/]"
    )
    console.print()
    while True:
