---
name: update-setup
description: "One-time setup wizard for the nanoinfra upgrade skill. Triggers: setup update, configure update, 设置更新, 初始化更新."
---

# Update Setup

Generate a personalized upgrade skill in Nanoinfra's agent workspace.
...
Use `exec` to run `nanoinfra --version`. Tell the user the current version.

Then collect install clues with `exec`. These commands are best-effort; if one fails,
keep going and show the useful output:

```
command -v nanoinfra || true
python -m pip show nanoinfra || true
pipx list | sed -n '/nanoinfra/,+3p' || true
uv tool list | sed -n '/nanoinfra/,+3p' || true
```
...
answer, stop and ask the user to rerun this setup when they know how nanoinfra was
installed.
...
`question: "Where is your nanoinfra source checkout? Enter an absolute path or a path relative to this workspace:"`.
...
| Method | Command |
|--------|---------|
| uv | `uv tool install "nanoinfra[EXTRAS]" --force` |
| pipx | `pipx install --force "nanoinfra[EXTRAS]"` |
| pip | `python -m pip install --upgrade "nanoinfra[EXTRAS]"` |
...
---
name: update
description: "Upgrade nanoinfra to the latest version. Triggers: upgrade nanoinfra, update nanoinfra, 升级nanoinfra, 更新nanoinfra."
---
...
4. Use `exec` to verify: `nanoinfra --version`
5. Tell the user the new version. Say: "Run `/restart` to restart nanoinfra and apply the update. If `/restart` is unavailable in this channel, restart the nanoinfra process manually."
```

## Step 5: Confirm

Only after `write_file` succeeds, tell the user:
"Upgrade skill created. Say 'upgrade nanoinfra' when you want to update."
