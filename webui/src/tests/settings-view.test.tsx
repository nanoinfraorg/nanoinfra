  ...
  render(
    <ClientProvider client={{} as never} token="tok">
      <SettingsView
        ...
        onOpenMetrics={options.onOpenMetrics}
      />
    </ClientProvider>,
  );
}
...
describe("SettingsView Apps catalog", () => {
  ...
  it("points at Metrics instead of carrying the usage band it used to own", async () => {
    // The summary and the heatmap moved to their own destination (#235). The overview keeps one
    // row so an operator who knows where the numbers were finds where they went, and the
    // five-second usage poll that fed the band is gone with it.
    const payload: SettingsPayload = {
      ...settingsPayload(),
      usage: {
        days: [],
    ...
    renderSettingsView({ initialSection: "overview", initialSettings: payload, onOpenMetrics });

    const row = await screen.findByTestId("overview-metrics");
    expect(within(row).getByText("2 calls in the last 30 days")).toBeInTheDocument();
    fireEvent.click(row);
    expect(onOpenMetrics).toHaveBeenCalled();

    expect(screen.queryByLabelText("Token activity")).not.toBeInTheDocument();
    expect(
      fetchMock.mock.calls.some(([input]) => String(input).startsWith("/api/settings/usage")),
    ).toBe(false);
  });
