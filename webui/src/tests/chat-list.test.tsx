  ...
  it("groups WebUI chats by workspace project while preserving in-project sorting and activity", () => {
    const sessions = [
      session({
        ...
        workspaceScope: {
          project_path: "/Users/me/nanoinfra",
          project_name: "nanoinfra",
      ...
      session({
        ...
        workspaceScope: {
          project_path: "/Users/me/nanoinfra-bench",
          project_name: "nanoinfra-bench",
          access_mode: "full",
        },
      }),
    ];
    ...
    const nanoinfraSection = screen.getByRole("region", { name: "nanoinfra" });
    const nanoinfraText = nanoinfraSection.textContent ?? "";

    expect(screen.getByRole("region", { name: "nanoinfra-bench" })).toBeInTheDocument();
    expect(within(nanoinfraSection).getByText("Alpha task")).toBeInTheDocument();
    expect(within(nanoinfraSection).getByText("Zeta task")).toBeInTheDocument();
    expect(nanoinfraText.indexOf("Alpha task")).toBeLessThan(nanoinfraText.indexOf("Zeta task"));
    expect(within(nanoinfraSection).getByLabelText("Agent running")).toBeInTheDocument();
  ...
  it("keeps default workspace topics in the Topics section instead of a project folder", () => {
    const sessions = [
      session({
        ...
        updatedAt: "2026-05-21T10:00:00Z",
        workspaceScope: {
          project_path: "/Users/me/.nanoinfra/workspace",
    ...
    render(
      <ChatList
        ...
        onRequestRename={vi.fn()}
        onToggleArchive={vi.fn()}
        defaultWorkspacePath="/Users/me/.nanoinfra/workspace"
        showTimestamps
      />,
    );

    expect(screen.getByText("Projects")).toBeInTheDocument();
    expect(screen.getByRole("region", { name: "nanoinfra" })).toBeInTheDocument();
  ...
  it("can collapse a project group and keeps project rename separate from chat titles", async () => {
    ...
    render(
      <ChatList
        ...
        projectNameOverrides={{ "/Users/me/nanoinfra": "Photos" }}
        collapsedGroups={{ "project:/Users/me/nanoinfra": true }}
      />,
    );
    ...
    expect(onToggleGroup).toHaveBeenCalledWith("project:/Users/me/nanoinfra");
    ...
    expect(onNewChatInProject).toHaveBeenCalledWith("/Users/me/nanoinfra", "Photos");
    ...
    expect(onRequestRenameProject).toHaveBeenCalledWith("/Users/me/nanoinfra", "Photos");
  });
