...
interface DiagramDeleteConfirmProps {
  open: boolean;
  diagramName: string;
  onCancel: () => void;
  onConfirm: () => void;
...
/** Matches the app's chat-delete confirm (see DeleteConfirm.tsx) so deleting a diagram doesn't feel like a different product. */
export function DiagramDeleteConfirm({ open, diagramName, onCancel, onConfirm }: DiagramDeleteConfirmProps) {
  return (
      ...
      <AlertDialogContent className="w-[min(calc(100vw-2rem),24rem)] gap-0 rounded-[28px] border border-white/70 bg-card/95 p-5 text-center shadow-[0_24px_80px_rgba(15,23,42,0.20)] backdrop-blur-xl data-[state=open]:zoom-in-95 sm:rounded-[28px]">
        <AlertDialogHeader className="items-center space-y-0 text-center">
          ...
          <AlertDialogDescription className="mt-3 max-w-[17rem] text-center text-[14px] leading-6 text-muted-foreground">
            This removes it from local storage. There's no backend yet, so this can't be undone.
          </AlertDialogDescription>
        </AlertDialogHeader>
