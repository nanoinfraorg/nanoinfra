...
interface WorkspaceDeleteConfirmProps {
  ...
  name: string;
  /**
   * True once the server has answered that the folder is not empty. The second
   * pass says what the recursive delete actually removes, rather than asking the
   * same question twice -- the client never sends `recursive` speculatively.
   */
  recursive: boolean;
  onCancel: () => void;
  onConfirm: () => void;
...
/** Matches the chat and diagram delete confirms (see DeleteConfirm.tsx) so removing a file doesn't feel like a different product. */
      ...
      <AlertDialogContent className="w-[min(calc(100vw-2rem),24rem)] gap-0 rounded-[28px] border border-white/70 bg-card/95 p-5 text-center shadow-[0_24px_80px_rgba(15,23,42,0.20)] backdrop-blur-xl data-[state=open]:zoom-in-95 sm:rounded-[28px]">
        <AlertDialogHeader className="items-center space-y-0 text-center">
          ...
          <AlertDialogTitle className="text-center text-[20px] font-semibold leading-tight tracking-[-0.02em] text-foreground">
            {recursive ? `Delete “${name}” and everything in it?` : `Delete “${name}”?`}
          </AlertDialogTitle>
          <AlertDialogDescription className="mt-3 max-w-[17rem] text-center text-[14px] leading-6 text-muted-foreground">
            {recursive
              ? "This folder is not empty. Every file and folder inside it is removed from disk as well, and this cannot be undone."
              : "This removes it from disk, and it cannot be undone."}
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter className="mt-7 !grid grid-cols-1 gap-3 space-x-0 sm:grid-cols-2 sm:space-x-0">
          ...
          >
            {recursive ? "Delete everything" : "Delete"}
          </AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>
