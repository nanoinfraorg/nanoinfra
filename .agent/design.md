...
Runtime state fan-out follows the same boundary. `AgentLoop` may publish generic runtime events from `nanoinfra.bus.runtime_events` for turn/run/model/goal state changes, but WebUI/WebSocket wire details such as `_turn_end`, `_goal_status`, title refreshes, and goal-state sync belong in `nanoinfra.session.webui_turns.WebuiTurnCoordinator` or the relevant channel adapter.
